# Endpoint detection rules (KQL, tiered T1–T4)

**Domain: endpoint telemetry (Microsoft Defender `Device*` tables).** One of two
rule families in this repo — see [`../README.md`](../README.md) for the index and
the sibling `azure-control-plane/` family. These two families are deliberately
self-contained: each has its own `shared-functions.kql` (this one keys on
`DeviceName` + endpoint account noise; the Azure one keys on a service-principal
allow-list), because they read entirely different tables and share no join key.
